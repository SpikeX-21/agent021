...
class Tool(ABC):
    ...
    def to_cc_schema(self) -> Dict[str, Any]:
        """转换为 anthropic function calling schema 格式

        用于 直接在claude客户端能够直接调用，使工具能够被 claude 原生 function calling 使用

        Returns:
            符合 calude function calling 标准的 schema
        """
        parameters = self.get_parameters()
